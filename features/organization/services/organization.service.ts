...
function mapToCurrentOrganization(
  o: { id: string; name: string | null; operating_model?: string },
): CurrentOrganization {
  return {
    id: o.id,
    name: o.name ?? "",
...
export async function getOrganizationsForUser(): Promise<{
  ...
  if (!rpcError && rpcList.length) {
    const organizations: CurrentOrganization[] = rpcList.map((o: { id: string; name?: string | null }) =>
