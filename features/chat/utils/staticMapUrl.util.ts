...
export function buildStaticMapImageUrl(
  ...
  const mapbox = typeof process !== "undefined" ? process.env?.EXPO_PUBLIC_MAPBOX_TOKEN?.trim() : "";
  if (mapbox) {
    const lon = lng;
    const la = lat;
  ...
  const googleKey =
    typeof process !== "undefined" ? process.env?.EXPO_PUBLIC_GOOGLE_MAPS_ANDROID_KEY?.trim() : "";
  if (googleKey) {
