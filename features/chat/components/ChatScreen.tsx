            ...
            >
              {groupedTripRows.length === 0 ? (
                ...
                (() => {
                  ...
                  const hubChatFlow = chatEntry?.chatFlow ?? "private_trip";
                  const hubHasIndent = Boolean(
                    chatEntry?.indentId && String(chatEntry.indentId).trim(),
                  );
                  const hubIntegrated =
                    hubChatFlow === "integrated_group" || hubHasIndent;
                  const hubCounterparty = hubIntegrated
                    ? resolveCounterpartyPartyTypeForViewer(chatEntry ?? null, currentOrgId)
                    : null;
                  const hubPartyTypes: ConversationPartyType[] = !hubIntegrated
                    ? ["driver"]
                    : hubCounterparty
                      ? (["driver", hubCounterparty] as ConversationPartyType[])
                      : (["client", "supplier", "driver"] as ConversationPartyType[]);
                  const partyIconRow = (
                    <>
                      {hubPartyTypes.map((partyType) => {
...
function TripConversationDetailLoaded({
  ...
  const detailHeaderSubtitle = useMemo(() => {
    const indent =
      String(liveConv.indent_id ?? "").trim() ||
      String(liveTripEntry?.indentId ?? "").trim();
    const primaryRaw =
      formatChatPartyName(liveConv.party_name) ??
      displayPartyName(liveConv.party_type) ??
      "";
    const primaryName = primaryRaw.trim();

    if (!tripIsIntegrated || !indent) {
      return primaryName || undefined;
    }

    const oid = String(currentOrgId ?? "").trim();
    const clo = String(tripCompose?.client_linked_organization_id ?? "").trim();
    const slo = String(tripCompose?.supplier_linked_organization_id ?? "").trim();
    let partnerRaw = "";
    if (oid && clo && oid === clo) {
      partnerRaw =
        (tripCompose?.supplier_name ?? "").trim() ||
        (partyConversationMap.supplier?.party_name ?? "").trim();
    } else if (oid && slo && oid === slo) {
      partnerRaw =
        (tripCompose?.client_name ?? "").trim() ||
        (partyConversationMap.client?.party_name ?? "").trim();
    } else {
      const cp = resolveCounterpartyPartyTypeForViewer(liveTripEntry, currentOrgId);
      if (cp === "supplier") {
        partnerRaw =
          (tripCompose?.supplier_name ?? "").trim() ||
          (partyConversationMap.supplier?.party_name ?? "").trim();
    ...
    const partnerName = partnerRaw
      ? (formatChatPartyName(partnerRaw) ?? partnerRaw).trim()
      : "";
    if (partnerName && primaryName) return `${primaryName} · ${partnerName}`;
    return partnerName || primaryName || undefined;
  }, [
    tripIsIntegrated,
    liveConv.indent_id,
    liveConv.party_name,
    ...
    tripCompose?.client_name,
    tripCompose?.supplier_name,
    partyConversationMap.supplier,
    partyConversationMap.client,
    displayPartyName,
  ]);
  ...
  const headerPrimaryParty: ConversationPartyType =
    liveConv.conversation_type === "private_trip" && !hasTripIndent
      ? "driver"
      : liveConv.party_type;
