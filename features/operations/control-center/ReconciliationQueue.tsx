...
export function ReconciliationQueue({
  ...
  return (
    <Surface style={styles.surface} elevation={1}>
      <Text style={styles.title}>Reconciliation queue</Text>
      {items.length === 0 ? (
        <Text style={styles.empty}>No reconciliation mismatches.</Text>
      ) : (
        items.map((item) => (
          <View key={item.id} style={styles.rowWrap}>
            <OperationalListRow
              title={`${item.trip?.tripLabel ?? "Unlinked"} · ${item.sourceType.toUpperCase()}`}
              subtitle={`${item.postingState ?? "pending"}${item.retryCount > 0 ? ` · retries ${item.retryCount}` : ""}`}
              trailing={
                <Pressable
