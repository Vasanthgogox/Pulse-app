-- Add trip_organization_name to the bootstrap output so the supplier viewer can display
-- the indent owner's org name on the relabeled CLIENT tab.
--
-- The field joins organizations.name via trips.organization_id.
...
COMMENT ON FUNCTION public.get_initial_chat_state(UUID, INT, INT, INT, TEXT) IS
  ...
  'Includes trip_organization_name (org name of the trip fleet owner) for supplier viewer relabeling.';
