-- Fix repeated false pages from the DB health monitor.
--
-- Root cause 1 (heartbeat self-reference): run_db_health_monitor read its own
-- PREVIOUS heartbeat and alerted "monitor has not run in >30 min" from inside
-- the very run that proves it is alive. Any historical gap (DB restart,
-- maintenance window, a cycle skipped under connection pressure) made it page
-- on the next successful run, reporting a stale "Last success" that no longer
-- reflected reality. A job cannot reliably report its own downtime.
--   Fix: remove the self-alert from run_db_health_monitor (it still writes the
--   heartbeat), and add a separate watchdog cron job that reads the heartbeat
--   and pages once per stale episode.
--
-- Root cause 2 (transient-restart miscounted as failures): the consecutive-fail
-- counter for send-push-notifications (jobid 1) counted 'server restarted' rows
-- (which have NULL start_time) as failures, turning one transient restart into
-- a "failed 3x consecutively" page.
--   Fix: ignore 'server restarted' and NULL-start_time rows in the fail count.

-- ── De-dup state for the watchdog (page once per stale episode) ──────────────
CREATE TABLE IF NOT EXISTS public._monitor_watchdog_state (
  id             BOOLEAN PRIMARY KEY DEFAULT TRUE,  -- singleton row
  alerted_at     TIMESTAMPTZ,                       -- when we last paged for staleness
  CONSTRAINT _monitor_watchdog_singleton CHECK (id)
);

-- ── 1. Rewrite run_db_health_monitor: drop self-heartbeat-alert, fix fail count ──
...
BEGIN
  ...
  -- ── 0. HEARTBEAT (write only) ────────────────────────────────────────────
  -- We no longer self-report staleness here; a job cannot reliably detect its
  -- own downtime. The separate run_monitor_watchdog() job does that. We only
  -- record liveness for the watchdog to read.
  ...
  -- Exclude transient 'server restarted' rows (NULL start_time): a DB restart
  -- is not a send-push failure and must not accumulate into the count.
  ...
  FROM (
    ...
    WHERE jobid = 1
      ...
      AND return_message IS DISTINCT FROM 'server restarted'
...
-- ── 2. Watchdog: separate job that reports monitor liveness ─────────────────
-- Reads the heartbeat run_db_health_monitor writes. Pages only when the
-- heartbeat is genuinely stale (>30 min), and only ONCE per stale episode
-- (guarded by _monitor_watchdog_state.alerted_at) so a real stall pages once,
-- not every cycle. Clears the guard when the monitor recovers.
CREATE OR REPLACE FUNCTION public.run_monitor_watchdog()
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  SELECT alerted_at INTO v_alerted_at
  FROM public._monitor_watchdog_state WHERE id = TRUE;

  v_is_stale := v_last_run IS NULL OR v_last_run < NOW() - INTERVAL '30 minutes';

  IF v_is_stale AND v_alerted_at IS NULL THEN
    -- Newly stale: page once and remember we did.
    PERFORM net.http_post(
      url := v_slack_url,
      headers := jsonb_build_object('Content-Type', 'application/json'),
      body := jsonb_build_object(
        'text',
        ':rotating_light: *SUPABASE DB — CRITICAL ALERT* :rotating_light:' || E'\n\n'
        || ':red_circle: *CRITICAL — Health monitor has not run in >30 min*' || E'\n'
        || '  • Last run: ' || COALESCE(v_last_run::TEXT, 'never') || E'\n'
        || '  • Action: check cron job 2 (cron-health-alert) in cron.job' || E'\n\n'
        || '_' || NOW()::TEXT || '_'
      )::jsonb
    );

    INSERT INTO public._monitor_watchdog_state (id, alerted_at)
    VALUES (TRUE, NOW())
    ON CONFLICT (id) DO UPDATE SET alerted_at = EXCLUDED.alerted_at;

  ELSIF NOT v_is_stale AND v_alerted_at IS NOT NULL THEN
    -- Recovered: clear the guard and post an all-clear.
    UPDATE public._monitor_watchdog_state SET alerted_at = NULL WHERE id = TRUE;
    ...
    PERFORM net.http_post(
      ...
      body := jsonb_build_object(
        'text',
        ':white_check_mark: *SUPABASE DB — RECOVERED* — health monitor is running again (last run '
        || v_last_run::TEXT || ')'
      )::jsonb
    );
  END IF;
...
-- ── 3. Schedule the watchdog every 15 min (offset from the 10-min monitor) ──
SELECT cron.schedule(
  'monitor-watchdog',
  '*/15 * * * *',
  'SELECT public.run_monitor_watchdog_guarded();'
);
