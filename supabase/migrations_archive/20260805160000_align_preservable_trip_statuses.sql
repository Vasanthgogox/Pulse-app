-- Align assign_aggregate_trip_driver mid-trip preserve list with
-- features/trips/utils/tripPreservableStatuses.util.ts (PRESERVABLE_TRIP_STATUSES).
  ...
  SET
    ...
    status = CASE
      ...
      WHEN v_status IN (
        ...
        'pod_pending', 'pod_received', 'arrived', 'at_destination', 'confirmed_arrival'
      ) THEN v_trip.status
