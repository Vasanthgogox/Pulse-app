-- Merge manual client/supplier rows on connection approval by phone only (no org-name fallback).
-- Aligns with OTP flows: exact strip match OR last 10 digits when both sides have >=10 digits.
--
-- Builds on 20260425113000_fix_connection_approval_client_placeholder.sql (placeholder phone = linked-<other_org_id>).
