-- Score-tiered random tie-break for discover_organizations (top tier score >= 4 stays deterministic).
...
CREATE INDEX IF NOT EXISTS idx_organizations_new_discover
  ON public.organizations (created_at)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_connection_requests_new
  ON public.connection_requests (created_at, status)
  WHERE status IN ('pending', 'approved');

CREATE INDEX IF NOT EXISTS idx_indents_new
  ON public.indents (created_at)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_posts_new
