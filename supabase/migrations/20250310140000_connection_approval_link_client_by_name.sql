-- When a connection is approved, link existing client by name if no phone match.
-- This ensures manual clients (e.g. "Mukunt's organization") get linked_organization_id
-- so Compare & Verify works without needing another backfill. One-time backfill
-- (20250310130000) remains for data created before this trigger change.
...
DECLARE
  ...
  v_linked_phone text;
BEGIN
  -- Placeholder phone for linked-org client rows (clients.phone is NOT NULL and unique per org)
  ...
  IF v_already_supplier IS NULL THEN
    ...
    IF v_supplier_linked = 0 THEN
      -- Link existing supplier by name (manual supplier created before connection)
  ...
  IF v_already_client IS NULL THEN
    ...
    IF v_client_linked = 0 THEN
      -- Link existing client by name (manual client created before connection; Compare & Verify)
