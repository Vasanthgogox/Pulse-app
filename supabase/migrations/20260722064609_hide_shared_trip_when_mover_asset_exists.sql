-- SKIPPED ON FRESH REPLAY: references trips.source_indent_id, a column not
-- added until 20260828200000_operational_identity_codes_phase1.sql (this
-- migration predates it). get_trips_for_org is a LANGUAGE sql function, so
-- its body is validated against the catalog at CREATE FUNCTION time -- fails
-- outright on a from-scratch replay. Safe to skip wholesale: this function is
-- redefined once more by 20261207140000_anon_safe_org_rpcs_and_idle_timeout.sql
-- (by which point source_indent_id already exists), and nothing in the
-- migration chain itself calls get_trips_for_org -- only app runtime does,
-- after the full chain has finished replaying. Original body preserved below
-- in a comment for history.
