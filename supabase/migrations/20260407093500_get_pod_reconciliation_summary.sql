  ...
  SELECT 
    ...
    COUNT(t.id) FILTER (
      WHERE (t.invoice_no IS NULL OR t.invoice_no = '')
        ...
        AND (t.invoice_status_1 IS NULL OR (t.invoice_status_1 NOT ILIKE '%pending%' AND t.invoice_status_1 NOT ILIKE '%data shared%'))
    ) as received_count,
    COALESCE(SUM(t.client_price) FILTER (
      WHERE (t.invoice_no IS NULL OR t.invoice_no = '')
        AND (t.invoice_status_1 IS NULL OR t.invoice_status_1 != 'Raised')
    ...
    -- Invoiced
    COUNT(t.id) FILTER (
      WHERE (t.invoice_no IS NOT NULL AND t.invoice_no != '') OR t.invoice_status_1 = 'Raised'
    ) as invoiced_count,
    COALESCE(SUM(t.client_price) FILTER (
      WHERE (t.invoice_no IS NOT NULL AND t.invoice_no != '') OR t.invoice_status_1 = 'Raised'
    ), 0) as invoiced_sum
