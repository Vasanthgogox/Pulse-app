-- Primitive A: append Commerce catalog fields for driver verify.
-- Signature change requires DROP. Same SECURITY DEFINER, no new tables,
-- no Driver RLS on products. product_* is display-only (name / sku / image).

DROP FUNCTION IF EXISTS public.get_driver_trip_stop_orders(uuid);

CREATE FUNCTION public.get_driver_trip_stop_orders(p_trip_id uuid)
RETURNS TABLE (
  trip_id uuid,
  indent_id uuid,
...
COMMENT ON FUNCTION public.get_driver_trip_stop_orders(uuid) IS
  ...
  'product_id / product_name / product_sku / product_image_path are catalog display fields from '
  'sales_order_lines → products (image_path is org-assets). Absent when no line or no product. '
