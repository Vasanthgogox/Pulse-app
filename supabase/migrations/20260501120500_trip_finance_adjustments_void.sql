-- Soft-void trip finance adjustments: retain row for audit, exclude from totals via app logic.

ALTER TABLE public.trip_finance_adjustments
  ADD COLUMN IF NOT EXISTS voided_at timestamptz,
  ADD COLUMN IF NOT EXISTS void_reason text;

COMMENT ON COLUMN public.trip_finance_adjustments.voided_at IS
  'When set, adjustment is excluded from adjusted sale/cost totals but shown struck-through in UI.';
COMMENT ON COLUMN public.trip_finance_adjustments.void_reason IS
...
COMMENT ON FUNCTION public.get_shared_trip_finance_adjustments(uuid, uuid) IS
  'Compare & Verify: adjustments for caller org trips + partner org trips (bilateral tx) + mission_key from both books. Includes void audit fields.';
