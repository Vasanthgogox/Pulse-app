-- Tighten FO fleet-channel Reach bridge: LOAD only (no empty/unknown snapshot types).
-- Companion to 20270210153000 (already applied with a looser '' fallback).
...
COMMENT ON FUNCTION public.get_driver_reach_stories() IS
  'Driver Story tab: driver-channel boosted stories for all drivers; Fleet Owners also receive fleet-channel boosted LOAD stories only (snapshot_post_type = LOAD). Organic FO capacity is separate.';
...
COMMENT ON FUNCTION public.submit_driver_direct_bid(uuid, numeric, text) IS
  'Independent driver (incl. Fleet Owner) bids on a live Reach story as themselves. Driver-channel for all drivers; fleet-channel only when type/snapshot is LOAD and caller is_driver_fleet_owner.';
