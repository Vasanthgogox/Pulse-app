-- trip_feedback_status on bootstrap rows (get_initial_chat_state → unified B2B bootstrap).
-- Values: 'none' | 'pending' | 'rated' — client Zustand mirrors on PartyConv.feedbackStatus.
        ...
        'trip_feedback_status', (
          CASE
            WHEN EXISTS (
              ...
              WHERE  r.trip_id = tc.trip_id
                AND (
                  (tc.party_type = 'client' AND r.rated_type = 'client'
                    AND r.rated_id IS NOT DISTINCT FROM tc.client_id)
                  OR (tc.party_type = 'supplier' AND r.rated_type = 'supplier'
                    AND r.rated_id IS NOT DISTINCT FROM tc.supplier_id)
                  OR (tc.party_type = 'driver' AND r.rated_type = 'driver'
                    AND r.rated_id IS NOT DISTINCT FROM tc.driver_id)
                )
            )
            OR EXISTS (
              SELECT 1
...
COMMENT ON FUNCTION public.get_initial_chat_state(UUID, INT) IS
  'Bootstrap conversation rows + last N messages per lane. Includes trip_feedback_status '
  '(none|pending|rated) for B2B chat feedback card visibility without extra queries.';
