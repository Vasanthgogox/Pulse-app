-- Phase 1 of the idx_drivers_phone_normalised 23505 fix.
--
-- Root cause (2026-09-07 incident, 5x duplicate-key errors in ~40s):
-- idx_drivers_phone_normalised is a GLOBAL partial unique index --
--   (phone_normalised) WHERE phone_normalised IS NOT NULL AND user_id IS NULL AND left_at IS NULL
-- i.e. "at most one unclaimed driver placeholder per phone number, across
-- every organization" -- but assign_aggregate_trip_driver's existence check
-- before INSERT is scoped to `organization_id = p_driver_org_id`. If org A
-- already has an unlinked roster placeholder for a phone, org B assigning
-- an aggregate trip to that same phone passes its own (org-scoped) check,
-- then hits a raw 23505 on the INSERT. No advisory lock guards this either,
-- so even same-org concurrent calls could race.
--
-- Fix mirrors the already-correct pattern in accept_driver_invite
-- (pg_advisory_xact_lock -> re-check -> INSERT ... ON CONFLICT DO NOTHING
-- -> lost-race fallback), adapted for a GLOBAL (not org-scoped) lock and
-- conflict target, since that's what the actual constraint requires. The
-- lock key is namespaced ('assign_aggregate_trip_driver:phone:' || v_last10)
-- so it can't collide with unrelated advisory-lock usage elsewhere that
-- might hash similar strings for a different purpose.
--
-- Only the v_profile_id IS NULL branch is at risk: when exactly one
-- profiles row matches, the INSERT sets user_id = v_profile_id (NOT NULL),
-- which never matches the partial index's `user_id IS NULL` predicate, so
-- that branch was never exposed to this bug.
--
-- No behavior change for the success paths: same-org reuse, name-update-on-
-- reuse, vehicle/status/OTP logic are all untouched. The only new outcome
-- is a clean `{"ok": false, "error": "..."}` instead of an unhandled 23505
-- when a different org already holds an unlinked placeholder for this phone
-- -- deliberately NOT auto-adopting that row, since a drivers row represents
-- one org's employment relationship, not a global driver identity.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    -- phone number globally before deciding whether to insert.
    PERFORM pg_advisory_xact_lock(hashtextextended('assign_aggregate_trip_driver:phone:' || v_last10, 0));
    ...
    IF NOT FOUND THEN
      ...
      IF v_profile_id IS NULL THEN
        -- This INSERT would set user_id IS NULL, matching the global
        -- partial index's predicate. If any other org already holds an
        -- unlinked, active placeholder for this phone, surface a clear
        -- error instead of an unhandled 23505 -- we deliberately do not
        -- auto-adopt another org's row here (a drivers row is one org's
        -- employment relationship, not a global driver identity).
        IF EXISTS (
      ...
      IF v_driver.id IS NULL THEN
        -- Lost the race despite the lock (e.g. a differently-formatted
        -- phone from another session that normalizes the same way, applied
        -- between our EXISTS check and this INSERT). Re-check our own org;
        -- if still not ours, surface the same cross-org error rather than
        -- letting the caller see a raw constraint violation.
        SELECT * INTO v_driver
        FROM public.drivers
        WHERE organization_id = p_driver_org_id
          AND (
  ...
  SET
    ...
    updated_at = now()
  WHERE id = p_trip_id;
  -- Only a not-yet-linked driver needs the claim OTP -- it exists to prove
  -- phone ownership and link the account. An already-linked driver has
  -- nothing left to prove; generating a code here would produce one the
  -- dispatcher sees but the driver's app never needs or prompts for.
