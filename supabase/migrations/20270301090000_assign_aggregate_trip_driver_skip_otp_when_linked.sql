-- Only generate a trip claim OTP when the assigned driver row has no linked
-- user account yet. The OTP's sole purpose is to prove "this phone number
-- belongs to this real person" and link their account — for a driver who is
-- already linked (drivers.user_id IS NOT NULL, e.g. from a prior trip),
-- there is nothing left to prove, and generating/displaying a code that
-- will never be consumed is misleading to the dispatcher (it implies a
-- pending security step that doesn't actually apply).
--
-- Also returns `driver_linked` so the client can skip its own redundant
-- generateTripOtp(trip.id) call and the "Trip claim code" screen entirely
-- for an already-linked driver, instead of generating a second code that
-- would also never be used.
