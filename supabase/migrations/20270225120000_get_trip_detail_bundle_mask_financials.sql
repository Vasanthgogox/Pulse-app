-- Security fix: get_trip_detail_bundle() returned client_price, margin,
-- platform_fee, driver_commission, amount_paid, advance_paid, payment_status
-- unconditionally to any authorized viewer (owner, linked client, linked
-- supplier), with no per-field role masking. A linked supplier assigned to a
-- non-indent (manual/Aggregate-module) trip could see the owner's client
-- billing rate and internal margin — financial data that belongs only to the
-- owning organization. TripDetailScreen.tsx's isPartnerSettlementView flag
-- was meant to hide this client-side, but only activated for indent_id-based
-- trips, so manual/Aggregate trips fell through unmasked.
--
-- Masking must live here (the SECURITY DEFINER boundary), not only in the
-- client — this migration adds it as the source of truth; TripDetailScreen.tsx
-- is a display-correctness follow-up, not the security control.
--
-- Visibility rule (mirrors the existing, already-correct pattern in
-- get_trips_for_org — see "Owner-only finance" CASE there, and the
-- "Supplier's own commercial terms" block for supplier_rate/advance_paid):
--   client_price      -> owner + linked client (their own billed rate)
--   supplier_rate     -> owner + linked supplier (their own settlement)
--   margin            -> owner only
--   platform_fee      -> owner only
--   driver_commission -> owner only (partner-owns-asset exception is Issue B's
--                         scope — a future execution_type signal, not this fix)
--   amount_paid       -> owner only
--   advance_paid      -> owner + linked supplier (advance paid TO the supplier)
--   payment_status    -> owner only
--
-- Row-level trip access (organization_members membership check, and the
-- owner/supplier/client EXISTS clause selecting which trips are visible at
-- all) is unchanged — this only narrows which financial columns are
-- populated within an already-authorized row.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'trip', jsonb_build_object(
      ...
      'driver_commission',           CASE WHEN v_is_owner THEN v_driver_commission END,
