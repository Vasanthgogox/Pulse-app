-- Partner display: expose fleet asset count + indents shared to the network
-- for network profile stats (RLS blocks direct vehicle/indent reads on other orgs).
...
COMMENT ON FUNCTION public.get_connection_partner_display_batch(uuid[]) IS
  'Batch partner display: branding, stats, KYC verificationStatus, asset vehicleCount, networkIndentCount. SECURITY DEFINER.';
...
COMMENT ON FUNCTION public.get_connection_partner_display(uuid) IS
  'Linked org display including KYC verificationStatus, asset vehicleCount, networkIndentCount. SECURITY DEFINER.';
