...
-- CONCURRENTLY is used to avoid locking tables during index build.
-- Note: CONCURRENTLY cannot run inside a transaction block; each statement is independent.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_trips_driver_org
  ON trips(driver_id, organization_id) WHERE deleted_at IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_trips_status_completed
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_drivers_id_commission
