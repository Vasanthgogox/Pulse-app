-- Reassign on aggregate trips must not rewind in_transit / in_progress status to assigned.
...
COMMENT ON FUNCTION public.assign_aggregate_trip_driver(uuid, uuid, text, text) IS
  'Assign driver to aggregate trip by phone; preserves mid-trip status on reassignment.';
