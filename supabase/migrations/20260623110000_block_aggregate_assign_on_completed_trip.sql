-- Refuse assign_aggregate_trip_driver once the trip is completed (aligns with
-- TripAssignmentBlock + trips.service updateTripAssignment terminal guard).
...
COMMENT ON FUNCTION public.assign_aggregate_trip_driver(uuid, uuid, text, text) IS
  'Assign driver to aggregate trip by phone; refuses if driver already on another non-terminal trip or trip is completed.';
