-- Summaries-only bootstrap: add p_include_message_bodies (default TRUE).
-- FALSE omits embedded trip_messages per conversation (hydrate on thread open).
DROP FUNCTION IF EXISTS public.get_multi_lane_bootstrap(uuid, integer, integer, integer, text);
DROP FUNCTION IF EXISTS public.get_unified_b2b_bootstrap(uuid, integer, integer, integer, text);
DROP FUNCTION IF EXISTS public.get_b2b_chat_bootstrap(uuid, integer, integer, integer, text);
DROP FUNCTION IF EXISTS public.get_whatsapp_bootstrap_data(uuid, integer, integer, integer, text);
DROP FUNCTION IF EXISTS public.get_initial_chat_state(uuid, integer, integer, integer, text);

-- Hub trip lifecycle bucket on bootstrap: `active` (in-flight), `history` (completed/delivered/done),
-- or `all`. Enables a second fetch for HISTORY hub scope without loading the full fleet into `active`.
...
REVOKE ALL    ON FUNCTION public.get_initial_chat_state(UUID, INT, INT, INT, TEXT, BOOL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_initial_chat_state(UUID, INT, INT, INT, TEXT, BOOL) TO authenticated;

COMMENT ON FUNCTION public.get_initial_chat_state(UUID, INT, INT, INT, TEXT, BOOL) IS
  'Bootstrap conversations + messages; optional trip window; hub bucket active|history|all '
  '(completed/delivered/done for history). Ranking when windowed: unread > long-haul late > recency.';
...
REVOKE ALL    ON FUNCTION public.get_whatsapp_bootstrap_data(UUID, INT, INT, INT, TEXT, BOOL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_whatsapp_bootstrap_data(UUID, INT, INT, INT, TEXT, BOOL) TO authenticated;
...
REVOKE ALL    ON FUNCTION public.get_b2b_chat_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_b2b_chat_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) TO authenticated;
...
REVOKE ALL ON FUNCTION public.get_unified_b2b_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_unified_b2b_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) TO authenticated;
...
COMMENT ON FUNCTION public.get_multi_lane_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) IS
  'Multi-lane bootstrap: conversations + lanes + messages_by_context; optional trip window; hub bucket.';

REVOKE ALL ON FUNCTION public.get_multi_lane_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_multi_lane_bootstrap(UUID, INT, INT, INT, TEXT, BOOL) TO authenticated;
