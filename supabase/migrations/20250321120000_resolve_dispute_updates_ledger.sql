-- 1) accept_partner_view: when receiver has no prior transaction, set contact_type from clients/suppliers
--    so the new row is attributed correctly (e.g. client's "payment to supplier" has contact_type = 'supplier').
-- 2) resolve_dispute RPC: on ACCEPT, update receiver's ledger via accept_partner_view then mark dispute RESOLVED.
--    Ensures "Accept & update" actually updates the client's book even if app had stale/missing raised_sales/raised_paid.

-- 1) Fix accept_partner_view: derive contact_type when first_contact_type is null
...
COMMENT ON FUNCTION public.accept_partner_view(uuid, uuid, numeric, numeric, uuid) IS
  'Self-correct: update caller org ledger for trip to match partner_sales/partner_paid. Used by Update my book and resolve_dispute (Accept dispute).';

-- 2) resolve_dispute: on ACCEPT, update receiver ledger then set status RESOLVED
CREATE OR REPLACE FUNCTION public.resolve_dispute(
  p_dispute_id uuid,
  p_action text,
  p_resolved_by_org_id uuid
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_dispute public.dispute%ROWTYPE;
  v_contact_id uuid;
BEGIN
  IF p_action IS NULL OR p_resolved_by_org_id IS NULL THEN
    RETURN;
  END IF;
  IF NOT public.is_org_member(p_resolved_by_org_id) THEN
    RETURN;
  END IF;
  ...
  WHERE d.id = p_dispute_id
    AND d.partner_org_id = p_resolved_by_org_id
    AND d.status = 'OPEN';
  ...
  IF p_action = 'ACCEPT' THEN
    -- Resolve receiver's contact that represents the raiser (for accept_partner_view).
    SELECT c.id INTO v_contact_id
    FROM public.clients c
    WHERE c.organization_id = v_dispute.partner_org_id
      AND c.linked_organization_id = v_dispute.raised_by_org_id
    LIMIT 1;
    IF v_contact_id IS NULL THEN
      SELECT s.id INTO v_contact_id
      FROM public.suppliers s
      WHERE s.organization_id = v_dispute.partner_org_id
        AND s.linked_organization_id = v_dispute.raised_by_org_id
      LIMIT 1;
    END IF;

    -- Update receiver's ledger to match raiser's view (raised_sales, raised_paid).
    PERFORM public.accept_partner_view(
      v_dispute.partner_org_id,
      v_dispute.transaction_id,
      COALESCE(v_dispute.raised_sales, 0),
      COALESCE(v_dispute.raised_paid, 0),
      v_contact_id
    );
...
COMMENT ON FUNCTION public.resolve_dispute(uuid, text, uuid) IS
  'Resolve dispute: DECLINE only marks RESOLVED; ACCEPT updates receiver ledger to match raiser (raised_sales/raised_paid) then marks RESOLVED.';

REVOKE ALL ON FUNCTION public.resolve_dispute(uuid, text, uuid) FROM PUBLIC;
