-- Phase 3A: read-only open marketplace loads for Driver App Fleet Owners.
-- Demand remains Business-created indents. No bidding / trip create.
...
BEGIN
  ...
  SELECT
    ...
    -- Commercial offer for supply side (never client_price / client PII).
...
COMMENT ON FUNCTION public.list_open_marketplace_loads_for_fleet_owner(integer) IS
  'Phase 3A: sanitized open marketplace/both indents for Driver App Fleet Owners. Read-only; no bidding.';
