-- discover_organizations: return partner avatars (org logo → owner profile photo → seed/initials).
-- Matches get_connection_partner_display / network hub list resolution.
...
BEGIN
  ...
  base_candidates AS (
    SELECT
      ...
      COALESCE(NULLIF(TRIM(o.avatar_seed), ''), p.avatar_seed) AS avatar_seed
...
COMMENT ON FUNCTION public.discover_organizations(uuid, text, integer, integer) IS
  'Discover orgs for network growth. avatar_url = organizations.logo_url then owner profiles.avatar_url; avatar_seed from org or owner profile.';
