-- get_trip_detail_bundle ground_ops gate — same inlined-predicate approach
-- as 20270917100000_ground_ops_warehouse_scoping.sql, for consistency (one
-- warehouse-membership check pattern across the whole feature, no
-- SECURITY DEFINER helper function called per row/call — see that
-- migration's PERFORMANCE NOTE for why).
--
-- Body is otherwise identical to
-- 20270912143000_get_trip_detail_bundle_dco_operating_mode.sql, with the
-- ground_ops gate inserted right after the existing active-membership
-- check and before the trip SELECT (clean deny, no partial data).
...
DECLARE
  ...
  v_dco_payee_id               UUID;
  v_viewer_is_ground_ops       BOOLEAN;
  v_ground_ops_can_access      BOOLEAN;
BEGIN
  ...
  ) INTO v_viewer_is_ground_ops;

  IF v_viewer_is_ground_ops THEN
    SELECT EXISTS (
      SELECT 1
      FROM trips t
      JOIN organization_members om ON om.organization_id = t.organization_id
      JOIN organization_member_warehouses omw ON omw.organization_member_id = om.id
      JOIN indents i ON i.id = t.indent_id
      WHERE t.id = p_trip_id
        AND om.user_id = (SELECT auth.uid())
        AND om.organization_id = p_viewer_org_id
        AND om.status = 'active'
        AND (
          EXISTS (
    ...
    ) INTO v_ground_ops_can_access;

    IF NOT v_ground_ops_can_access THEN
      RETURN NULL;
    END IF;
  END IF;
  ...
  SELECT
    ...
    t.operating_mode, t.dco_payee_id
  INTO
    ...
    v_operating_mode, v_dco_payee_id
  FROM trips t
  WHERE t.id = p_trip_id
...
COMMENT ON FUNCTION public.get_trip_detail_bundle(uuid, uuid) IS
  'Trip detail bundle for Business App. Includes operating_mode and dco_payee_id (DCO discriminator/payee) without changing Asset vs Aggregate financial masking. Ground Ops viewers additionally gated by an inlined warehouse-membership EXISTS check.';
