-- Product decision (Issue A/B follow-up): a manually-created, Aggregate-
-- module-assigned trip (source='manual', indent_id IS NULL, supplier_id set)
-- must NOT be visible to the supplier's own linked org at all. Unlike the
-- indent/marketplace flow (trips_supplier_view, already correctly scoped to
-- indent_id IS NOT NULL), a manual assignment has no consent step on the
-- supplier's side — a dispatcher just enters a phone number and assigns.
-- Showing the trip in the supplier's own dashboard, even with masked
-- financials (see the two prior migrations), would let any org "spam" a
-- linked supplier with fabricated/unwanted trip assignments they never
-- agreed to. Settlement for this trip shape is handled offline between the
-- two orgs; the driver still sees the trip via trips_driver_view (unaffected
-- — driver access was never tied to organization membership).
--
-- get_trips_for_org: add indent_id IS NOT NULL to the supplier-visibility
-- branch of the row filter (does not touch the owner branch or the
-- financial-field CASE masking already in place).
--
-- get_trip_detail_bundle: add the same condition to the supplier EXISTS
-- clause in its row-visibility WHERE. Column-level masking from the prior
-- two migrations is left in place unchanged — it's still correct
-- defense-in-depth for any other path that might reach a supplier-visible
-- trip, and this migration is strictly an additional, narrower row filter
-- on top of it.
    ...
    WHERE public.is_org_member(p_org_id)
      AND (
        ...
        OR (
          ...
          -- Manual/Aggregate-module assignment (no indent) is never shown to
          -- the supplier's own org — no consent step exists for that path.
...
-- get_trip_detail_bundle: same row-visibility narrowing on the supplier branch.
