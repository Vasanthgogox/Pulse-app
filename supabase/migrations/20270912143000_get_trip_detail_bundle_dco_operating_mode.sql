-- Additive DCO fields on get_trip_detail_bundle.
-- Same function body as 20270226110000 plus trips.operating_mode and trips.dco_payee_id
-- on the trip JSON object. Unapplied until an isolated migration deployment exists.
-- Do not infer DCO from trip_payout_mode or supplier_id.
...
COMMENT ON FUNCTION public.get_trip_detail_bundle(uuid, uuid) IS
  'Trip detail bundle for Business App. Includes operating_mode and dco_payee_id (DCO discriminator/payee) without changing Asset vs Aggregate financial masking.';
