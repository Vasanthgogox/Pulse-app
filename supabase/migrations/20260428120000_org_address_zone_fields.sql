-- Add structured address + zone fields to organizations.
-- Also fixes handle_new_user: previously never wrote operating_model from metadata
-- (org always defaulted to HYBRID). Now writes operating_model + address fields.
...
ALTER TABLE public.organizations
  ...
  ADD COLUMN IF NOT EXISTS zone         text
    CONSTRAINT organizations_zone_check
    CHECK (zone IS NULL OR zone IN ('NORTH','SOUTH','EAST','WEST','NORTHEAST'));

-- Replace handle_new_user to fix operating_model + add address writes
...
begin
  ...
  if r = 'user' then
    ...
    if not org_exists then
      ...
      insert into public.organizations (owner_id, name, operating_model, address_line, city, state, zone)
      values (
        new.id,
        org_name,
        coalesce(
          nullif(trim(new.raw_user_meta_data->>'operating_model'), ''),
          'HYBRID'
        ),
