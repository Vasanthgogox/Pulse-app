-- RPC: list trips pre-assigned to current user's phone (last 10 digits) waiting for OTP claim.
-- Driver app uses this with get_pending_otp_claim_count to show "Trip(s) waiting for OTP" and list.
-- Same criteria as get_pending_otp_claim_count: trip_otps valid, driver.user_id IS NULL, driver phone matches.
...
COMMENT ON FUNCTION public.get_pending_otp_trips() IS 'Driver app: trips pre-assigned to current user phone (last 10 digits) waiting for OTP claim. SECURITY DEFINER.';
