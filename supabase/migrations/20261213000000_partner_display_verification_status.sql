-- Expose admin KYC verification_status on partner display RPCs so network
-- profiles can show a Verified badge when organizations.verification_status = verified.
-- Direct SELECT on organizations is often RLS-blocked for non-members.
...
COMMENT ON FUNCTION public.get_connection_partner_display_batch(uuid[]) IS
  'Batch partner display: branding, trip_count, average_rating, org tenure, owner signup, KYC verificationStatus. SECURITY DEFINER.';
...
COMMENT ON FUNCTION public.get_connection_partner_display(uuid) IS
  'Linked org display profile including tenure and KYC verificationStatus. SECURITY DEFINER.';
