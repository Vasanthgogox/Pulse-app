-- Reach-targeted orgs can retrieve sponsored loads in Load Center.
--
-- Problem: a load broadcast via a paid Reach campaign is visible in the story
-- feed to every targeted org (get_network_feed honours reach_campaign_targets),
-- but market_indents_for_org only admitted orgs via the supplier book
-- (via_link) or an existing award (via_award). A Reach-only target could
-- therefore see the story and place a bid, then find the load absent from Load
-- Center — unable to track, update, or negotiate its own pending offer.
--
-- Fix: third admission branch, via_reach. A campaign target sees the load while
-- the campaign is live, and indefinitely once it has actually bid — a pending
-- offer must not vanish mid-negotiation just because the campaign window
-- closed. Non-bidding targets lose visibility at expiry, as before.
--
-- Awarding an unconnected bidder is gated in the client (supplier invite must
-- be accepted first); this migration only restores read access.
...
-- Parameter renamed org_id -> p_org_id: `org_id` collided with
-- indents.organization_id / reach_campaign_targets.org_id inside the new
-- branch. Positional signature market_indents_for_org(uuid) is unchanged, so
-- every existing caller (supabase.rpc) is unaffected.
