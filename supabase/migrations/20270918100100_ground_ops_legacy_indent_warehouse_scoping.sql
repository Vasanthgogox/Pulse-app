-- Extend ground_ops warehouse resolution with a third path: a direct
-- indents.warehouse_id link (for legacy/native indents that have neither
-- sales_order_id nor execution_plan_id — see
-- 20270918100000_indents_warehouse_id_and_client_id_backfill.sql for the
-- column itself and the rationale).
--
-- Every place that currently checks "sales_order path OR execution_plan
-- path" gets a third branch: "OR i.warehouse_id = omw.warehouse_id".
-- Touches: trips_ground_ops_select_assigned, trip_documents SELECT/INSERT,
-- get_trips_for_org, get_trips_delta, get_trip_detail_bundle. Same inlined
-- EXISTS pattern as the rest of this feature (no SECURITY DEFINER helper
-- function per row — see 20270917100000's PERFORMANCE NOTE for why).
