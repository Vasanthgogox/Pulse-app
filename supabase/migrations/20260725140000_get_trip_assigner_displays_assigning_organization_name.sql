-- Driver app: include assigning fleet organization name in get_trip_assigner_displays_for_driver.
-- Drivers often cannot SELECT organizations for other fleets (RLS), so client-side org name
-- resolution fell back to "Assigning fleet" while the person (owner/dispatcher) still showed.
--
-- Return type gains a column; DROP + CREATE (CREATE OR REPLACE cannot change OUT row type).
...
BEGIN
  ...
  LOOP
    SELECT
      ...
      t.organization_id
    INTO v_driver_id, v_created_by, v_owner, v_assigned_by, v_organization_id
    FROM public.trips t
    WHERE t.id = r_tid;
    ...
    IF v_organization_id IS NOT NULL THEN
      SELECT nullif(trim(o.name), '') INTO v_org_name
    ...
    assigning_organization_name := v_org_name;
    RETURN NEXT;
  END LOOP;
...
COMMENT ON FUNCTION public.get_trip_assigner_displays_for_driver(uuid[]) IS
  'Driver app: for trips visible to this driver, return assigner person (audit → assigned_by_user_id → created_by/owner), assigner user id, and trips.organization_id fleet name. SECURITY DEFINER.';
