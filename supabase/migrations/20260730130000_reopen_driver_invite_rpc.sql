-- Re-open fleet driver invites (rejected/accepted) back to pending for re-connect flows.
-- Used when a disconnected driver should receive a new in-app invitation.
...
BEGIN
  ...
  SET
    ...
    payable_amount = coalesce(p_payable_amount, di.payable_amount),
    commission_percent = coalesce(p_commission_percent, di.commission_percent),
    commission_per_km = coalesce(p_commission_per_km, di.commission_per_km)
...
COMMENT ON FUNCTION public.reopen_driver_invite(uuid, uuid, text, text, numeric, numeric, numeric) IS
  'Fleet owner re-opens a rejected or accepted driver_invites row to pending (e.g. disconnected driver reconnect).';
