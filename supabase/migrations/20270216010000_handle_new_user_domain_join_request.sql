-- Move the company-email-domain collision check into handle_new_user itself so
-- it covers every signup path uniformly (email/password AND Google OAuth,
-- which auto-creates the org via this trigger before any client code runs).
--
-- When onboarding_type='owner' and the new user's email domain already has an
-- org (and isn't a free/public domain), skip org+membership provisioning and
-- file a pending organization_domain_join_requests row instead — surfaced to
-- that org's owner/admin under Team → Action needed.
...
BEGIN
  ...
  IF r = 'user' AND onboarding_type = 'owner' THEN
    ...
    -- Company-domain collision check: does another org's owner already use
    -- this email's domain? Free/public domains (gmail.com etc.) never match.
    v_email_domain := split_part(lower(trim(NEW.email)), '@', 2);
    IF v_email_domain <> '' AND NOT EXISTS (
      SELECT 1 FROM public.public_email_domains WHERE domain = v_email_domain
    ) THEN
      SELECT o.id, o.name INTO v_domain_org_id, v_domain_org_name
      FROM public.organizations o
      JOIN public.profiles p ON p.id = o.owner_id
      WHERE o.deleted_at IS NULL
        AND lower(trim(p.email)) LIKE '%@' || v_email_domain
      ORDER BY o.created_at ASC
      LIMIT 1;
    END IF;

    IF v_domain_org_id IS NOT NULL THEN
      INSERT INTO public.organization_domain_join_requests (
        organization_id, user_id, email, email_domain, requester_name
      ) VALUES (
        v_domain_org_id, NEW.id, lower(trim(NEW.email)), v_email_domain, display_name
      )
      ON CONFLICT (organization_id, user_id) WHERE status = 'pending'
      DO NOTHING;
      RAISE LOG 'handle_new_user: domain join request filed for user % against org % (domain %)',
        NEW.id, v_domain_org_id, v_email_domain;

      -- Skip org + membership provisioning entirely — this user joins the
      -- matched org once its owner/admin approves the request.
      RETURN NEW;
    END IF;
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Auth signup trigger: provisions profile; owner onboarding_type creates org + owner membership, unless the email domain already matches an existing org (files a join request instead); member/guest/etc. join via invitation accept.';

-- ── Client-side lookup: after signup, was a join request filed instead of an org? ──

CREATE OR REPLACE FUNCTION public.get_my_pending_domain_join_request()
  RETURNS TABLE(
    id                uuid,
  ...
  SET search_path = public
AS $$
  SELECT r.id, r.organization_id, coalesce(nullif(trim(o.name), ''), 'Organization'), r.status, r.created_at
...
GRANT EXECUTE ON FUNCTION public.get_my_pending_domain_join_request() TO authenticated;
