-- Ensure receiver org (to_organization_id) also has a supplier with linked_organization_id = requester (from_org).
-- So when the supplier initiates the connection, the client org gets a supplier contact for the supplier org,
-- enabling: (1) client to record payments to that supplier, (2) get_shared_ledger_trip_summary to resolve
-- counterparty_contact_id and return partner_paid (client's amount_out) in supplier view.
...
DECLARE
  ...
  v_receiver_supplier_linked int := 0;
  v_already_receiver_supplier int;
  v_linked_phone text;
BEGIN
  ...
  -- Receiver (to_org) must have a supplier with linked_organization_id = from_org so that:
  -- when from_org is the supplier and to_org is the client, the client can record payments to that supplier
  -- and get_shared_ledger_trip_summary can resolve counterparty and return partner_paid in supplier view.
  SELECT 1 INTO v_already_receiver_supplier
  FROM public.suppliers
  WHERE organization_id = new.to_organization_id AND linked_organization_id = new.from_organization_id
  LIMIT 1;

  IF v_already_receiver_supplier IS NULL THEN
    UPDATE public.suppliers s
    SET linked_organization_id = new.from_organization_id, supplier_type = 'integrated', updated_at = now()
    FROM public.organizations o
    WHERE o.id = new.from_organization_id
      AND s.organization_id = new.to_organization_id
      AND (s.linked_organization_id IS NULL OR s.linked_organization_id <> new.from_organization_id)
      AND trim(lower(coalesce(s.name, ''))) = trim(lower(coalesce(o.name, '')));
    GET DIAGNOSTICS v_receiver_supplier_linked = row_count;
    IF v_receiver_supplier_linked = 0 THEN
      INSERT INTO public.suppliers (organization_id, name, phone, email, linked_organization_id, supplier_type, updated_at)
      SELECT new.to_organization_id,
        coalesce(nullif(trim(o.name), ''), 'Connected'),
        NULL, NULL,
        new.from_organization_id, 'integrated', now()
      FROM public.organizations o WHERE o.id = new.from_organization_id;
      IF NOT FOUND THEN
        INSERT INTO public.suppliers (organization_id, name, linked_organization_id, supplier_type, updated_at)
        VALUES (new.to_organization_id, 'Connected', new.from_organization_id, 'integrated', now());
      END IF;
    END IF;
  ELSE
