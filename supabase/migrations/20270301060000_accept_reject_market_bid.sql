-- WS1 (Gate 4, step 7 of 7): accept_market_bid() / reject_market_bid().
--
-- accept_market_bid() locks the INDENT first (per the explicit concurrency
-- rule locked for this contract: "the shared award engine must lock the
-- Indent, not merely the individual bid"), then reuses the same shared
-- identity-resolution engine as accept_driver_direct_bid
-- (_resolve_or_create_market_driver, 20270301050000).
--
-- SCOPE NOTE, reported rather than silently decided: this migration
-- implements the award path for bidder_type='dco' only. market_bids already
-- supports bidder_type='organization' (a Business bidding its own capacity
-- into Market) per the locked product model, but the Gate 4 contract never
-- specified that bidder's trip-creation mechanics at the RPC level -- it is
-- a materially different shape from a DCO award (most likely closer to the
-- existing _ensure_mover_asset_trip / create_mover_asset_trip pattern: the
-- winning org gets a trip in its own books against its own roster, not a
-- driver-stub created under the awarding org). Rather than invent that
-- design now, accept_market_bid raises a clear not_implemented exception for
-- bidder_type='organization', so the gap is loud and explicit instead of
-- silently wrong. This should be a deliberate follow-up decision, not
-- something guessed here.
--
-- H4 (Gate 4 hardening, added after the first static review): acceptance
-- revalidates the bid's owner_vehicle_id (still exists, still owned by the
-- bidder, not soft-deleted) rather than trusting the ownership check already
-- performed at submission time, which can be stale by the time a bid is
-- actually accepted.

CREATE OR REPLACE FUNCTION public.reject_market_bid(p_bid_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.reject_market_bid(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reject_market_bid(uuid) TO authenticated;

COMMENT ON FUNCTION public.reject_market_bid(uuid) IS
  'Reject a pending market_bids row. Caller must be a non-driver active member of the org that owns the indent. No trip is created.';
...
BEGIN
  ...
  FROM public.trips t
  WHERE t.source = 'market_bid' AND t.source_bid_id = v_bid.id
  LIMIT 1;
  IF FOUND THEN
...
COMMENT ON FUNCTION public.accept_market_bid(uuid) IS
  'Accept a pending market_bids row (bidder_type=dco only -- see this migration''s header note on the organization bidder scope gap) and atomically create the trip via the shared _resolve_or_create_market_driver engine, locking the Indent first. Populates trips.indent_id and the real client_name, marks the indent awarded, and rejects competing market_bids -- the terminal-status trigger separately rejects competing bids/driver_direct_bids on any indent-linked post. Caller must be a non-driver active member of the organization that owns the indent.';
