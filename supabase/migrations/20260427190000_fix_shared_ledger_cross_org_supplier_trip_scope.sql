-- Fix shared-ledger trip scope for supplier-side cross-org views.
-- Scenario: viewer is supplier, but trips are owned by partner/client org.
-- Previous scope could miss these trips and return no partner rows, causing
-- "They record — / Awaiting partner" in trip command despite partner entries.
...
BEGIN
  ...
  WITH shared_viewer_trips AS (
    -- Viewer ledger-linked shared trips.
    ...
    -- Viewer-owned trips linked to this partner contact.
    ...
    -- Cross-org trips owned by partner org where counterparty is this viewer org.
    SELECT t.id AS trip_id
    FROM public.trips t
    WHERE t.organization_id = partner_org_id_val
      AND counterparty_contact_id IS NOT NULL
      AND (
        (is_partner_client AND t.supplier_id = counterparty_contact_id)
        OR (NOT is_partner_client AND t.client_id = counterparty_contact_id)
      )
  ),
...
COMMENT ON FUNCTION public.get_shared_ledger_entries(uuid, uuid) IS
  'Returns bilateral partner entries for shared trips, including cross-org supplier-view trips owned by partner org.';
...
BEGIN
  ...
  LEFT JOIN public.trips ptr
    ON ptr.id = swi.trip_id
   AND (ptr.organization_id = org_id OR ptr.organization_id = partner_org_id_val)
...
COMMENT ON FUNCTION public.get_shared_ledger_trip_summary(uuid, uuid) IS
  'Per-trip partner sales/paid for shared ledger, including cross-org supplier-view trips; sales uses supplier_rate.';
