-- List driver_direct_bids for a post owner (Business story bids sheet).
-- Returns display names + Fleet Owner flag so UI can show
-- "Fleet owner with driver (Name)" without reading profiles under RLS.

CREATE OR REPLACE FUNCTION public.list_driver_direct_bids_for_post(p_post_id uuid)
RETURNS TABLE (
  id                  uuid,
  post_id             uuid,
  driver_user_id      uuid,
...
BEGIN
  ...
  -- Caller must be an active member of the org that owns the post.
  IF NOT EXISTS (
    SELECT 1
    FROM public.posts p
...
COMMENT ON FUNCTION public.list_driver_direct_bids_for_post(uuid) IS
  'Shipper org members: list driver_direct_bids on their post with profile display name and is_fleet_owner for Story bids UI.';
