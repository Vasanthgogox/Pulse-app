...
-- =============================================================================
-- Evidence (production, cron.job_run_details jobid 15): every minute
--   ERROR 23502: null value in column "url" of relation "http_request_queue"
-- The scheduled command was:
--   net.http_post(url := current_setting('app.supabase_url', true) || '/functions/v1/log-watcher-scheduler', ...)
-- current_setting(..., true) is NULL when the GUC is unset, and NULL || text is NULL.
-- Same class of bug as 20261115000000 (dispatch_verification_workers).
--
-- Fix: wrap the call. If the URL (or vault service_role_key) is missing, return
-- without calling pg_net. Do not make http_request_queue.url nullable.
