-- Atomic aggregate assign (driver + optional fleet vehicle) + runtime migration gate for reassign.

CREATE OR REPLACE FUNCTION public.is_app_migration_applied(p_version text)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
COMMENT ON FUNCTION public.is_app_migration_applied(text) IS
  'Returns true when the given migration version is recorded on this database (reassign pre-ship gate).';

GRANT EXECUTE ON FUNCTION public.is_app_migration_applied(text) TO authenticated;
...
BEGIN
  ...
  IF lower(trim(coalesce(v_trip.status::text, ''))) IN ('completed', 'delivered', 'done')
    ...
    RETURN jsonb_build_object(
      'ok', false,
      'error',
      'Cannot change driver or vehicle after the trip is completed.'
    );
  END IF;
...
COMMENT ON FUNCTION public.assign_aggregate_trip_driver(uuid, uuid, text, text, uuid) IS
  'Assign aggregate trip driver by phone; optional fleet vehicle_id in same transaction; preserves mid-trip status.';
