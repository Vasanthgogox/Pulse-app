-- Bulk invitee lookup — same invariant as get_invitee_by_phone.
-- Contact-sync feeds this with the whole address book, so without the exclusion
-- every colleague surfaces as a connectable external org.
...
BEGIN
  ...
  -- Active members of the CALLER'S workspace org — excluded from all results.
  same_org_users AS (
    SELECT om.user_id
  ...
  prof AS (
    ...
    WHERE pr.phone IS NOT NULL
      ...
      AND pr.id NOT IN (SELECT user_id FROM same_org_users)
  ),
  profile_hits AS (
  ...
  auth_hits AS (
    ...
    WHERE i.norm NOT IN (SELECT ph.norm FROM profile_hits)
      AND au.id NOT IN (SELECT user_id FROM same_org_users)
