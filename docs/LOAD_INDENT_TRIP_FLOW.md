...
- **Supplier**: Receives the awarded load, creates the trip via "Assign Staff & Deploy" in Secured.
...
- **Supplier view** (AWARDED / SECURED): Shows "Assign Staff & Deploy". Supplier creates the trip here.

## Direct-Assignment Path

When indent has `assigned_supplier_id` (no quote flow): same semantics. Shipper does not create the trip. Supplier must create it from their Secured view (or equivalent flow for directly-assigned loads).

## UI Locations

- **Network → Load → HIRE PARTNER**: Shipper's loads (give + awarded pending trip). No Create Trip.
- **Network → Load → SECURED**: Supplier's awarded loads. "Assign Staff & Deploy" creates the trip.
