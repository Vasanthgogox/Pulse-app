...
export const CORE_CLIENT_COLUMNS =
  'id,organization_id,name,contact_person,phone,email,address,gstin,pan_number,status,created_at,updated_at,display_id,is_integrated,linked_organization_id,contact_percent,avatar_url,avatar_seed,owner_full_name';
...
export function mapRowToPlatformCustomer(row: Record<string, unknown>): PlatformCustomer {
  return {
    ...
    address: (row.registered_address as string | null) ?? (row.address as string | null) ?? null,
