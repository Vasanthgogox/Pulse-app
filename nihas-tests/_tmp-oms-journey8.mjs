...
async function main() {
  ...
  await shot(page, 'journey8-01-plan-history');

  // Click into EP-2026-001's row.
  const planRow = page.locator('table tbody tr').filter({ hasText: 'EP-2026-001' });
  await planRow.waitFor({ state: 'visible', timeout: 10000 });
  await planRow.click();
  await page.waitForTimeout(2000);
  console.log('url after clicking plan row:', page.url());
  await shot(page, 'journey8-02-plan-detail');
  const bodyText = await page.locator('body').innerText().catch(() => '');
  console.log('plan detail body:', bodyText.slice(0, 2000));
