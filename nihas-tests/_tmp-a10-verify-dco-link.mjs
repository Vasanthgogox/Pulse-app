...
(async () => {
  ...
  logs.length = 0;
  const dcoLink = page.getByLabel('DCO status', { exact: true }).first();
  await dcoLink.waitFor({ state: 'visible', timeout: 15000 });
  await dcoLink.click();
  await page.waitForTimeout(2500);
  await page.screenshot({ path: `${OUT}/a10-172-dco-status-screen-reachable.png`, fullPage: true });
