...
async function main() {
  ...
  await page.waitForTimeout(1500);
  await shot(page, 'journey11-00-edit-fields');

  const supplierTargetInput = page.locator('input').nth(3);
  await supplierTargetInput.fill('30000');
  await page.waitForTimeout(500);
  await shot(page, 'journey11-01-filled-supplier-target');

  await page.getByRole('button', { name: 'Save', exact: true }).first().click();
  await page.waitForTimeout(2000);
  await shot(page, 'journey11-02-after-save');
  const bodyText = await page.locator('body').innerText().catch(() => '');
  console.log('after save body:', bodyText.slice(0, 1500));
