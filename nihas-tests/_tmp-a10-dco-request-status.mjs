#!/usr/bin/env node
// A10 DCO Enrollment Phase 1: sign in as the existing A10 Pilot DCO,
// navigate to /dco-status, confirm current NONE state, tap "Request DCO
// status", capture the resulting screen. Real UI action only.
...
const OTP = '4321';
const log = (m) => console.log(`[dco-request] ${m}`);
...
async function main() {
  ...
  await page.screenshot({ path: `${OUT}/a10-169-profile-screen.png`, fullPage: true });
  log(`profile url: ${page.url()}`);

  const dcoStatusLink = page.getByLabel('DCO status', { exact: true }).first();
  await dcoStatusLink.waitFor({ state: 'visible', timeout: 15_000 });
  await dcoStatusLink.click();
  await page.waitForTimeout(2500);
  await page.screenshot({ path: `${OUT}/a10-170-dco-status-before.png`, fullPage: true });
  log(`dco-status url: ${page.url()}`);

  const requestBtn = page.getByText('Request DCO status', { exact: true }).first();
  const requestVisible = await requestBtn.isVisible().catch(() => false);
  log(`Request DCO status button visible (confirms NONE state): ${requestVisible}`);

  if (requestVisible) {
    await requestBtn.click();
    await page.waitForTimeout(3000);
    await page.screenshot({ path: `${OUT}/a10-171-dco-status-after-request.png`, fullPage: true });
  }
